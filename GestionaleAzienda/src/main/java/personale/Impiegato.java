...
public class Impiegato implements Serializable
{
  ...
  public Impiegato(String nome, String cognome, /*Sede*/String sedeLavorativa,
  ...
  public void SalvaSuFile(Impiegato persona)
  {
    ...
    // Se il codice fiscale della nuova persona è già esistente e non appartiene
    // alla persona da modificare annulla operazione
    if (CodiceFiscaleEsiste(persona.codiceFiscale)
        && !this.codiceFiscale.contentEquals(persona.codiceFiscale))
    {
      return;
    }
    ...
    // Salvo anche il ruolo della persona sul file
    ...
    try (BufferedReader fin = new BufferedReader(
        new FileReader(files.FILEIMPIEGATI)))
    {
      while ((riga = fin.readLine()) != null)
      {
        ...
        if (!riga.startsWith("#"))
        {
          ...
          try
          {
            ...
            // Se si modifica il codice fiscale si farà un controllo in base al
            // nome

            if (codiceFiscale.contentEquals(this.codiceFiscale.toLowerCase())
                || (nome.contentEquals(this.nome.toLowerCase())
                    && cognome.contentEquals(this.cognome.toLowerCase())))
            {
      ...
      try
      {
        ...
        fnuovo.write(
            "#nome,cognome,codiceFiscale,nomeSedeLavorativa,inizioAttivita,fineAttivita,ruolo"
    ...
    catch (FileNotFoundException e)
    {
      ...
      e.getMessage();
      System.out.println(
          "Impossibile trovare il file per il salvataggio degli impiegati!");
    }
    catch (IOException e)
    {
      e.printStackTrace();
  ...
  public static ArrayList<Impiegato> CaricaDaFile()
  {
        ...
        new FileReader(files.FILEIMPIEGATI)))
    {
      while ((riga = fin.readLine()) != null)
      {
        if (!riga.startsWith("#"))
        {
          ...
          {
            case 0:
              impiegati.add(new Operaio(elemento));
              break;
            case 1:
              impiegati.add(new Amministratore(elemento));
              break;
            case 2:
              impiegati.add(new Dirigente(elemento));
              break;
            case 3:
              impiegati.add(new Funzionario(elemento));
              break;
    ...
    catch (FileNotFoundException e)
    {
      ...
      e.getMessage();
      System.out.println(
          "Impossibile trovare il file per il caricamento degli impiegati!");
    }
    catch (IOException e)
    {
      e.printStackTrace();
      ...
      while ((riga = fin.readLine()) != null)
      {
        if (!riga.startsWith("#"))
        {
          ...
          if (codiceFiscale.contentEquals(elemento[2].toLowerCase()))
          {
            ...
            {
              case 0:
                return new Operaio(elemento);
              case 1:
                return new Amministratore(elemento);
              case 2:
                return new Dirigente(elemento);
              case 3:
                return new Funzionario(elemento);
              default:
                return new Operaio(elemento);
            }
          }
        ...
        new FileReader(files.FILEIMPIEGATI)))
    {
      while ((riga = fin.readLine()) != null)
      {
        if (!riga.startsWith("#"))
        {
          ...
          if (nome.toLowerCase().contentEquals(elemento[0].toLowerCase())
              && cognome.toLowerCase().contentEquals(elemento[1].toLowerCase()))
          {
            switch (ruolo)
            {
              case 0:
   ...
   */
  public static ArrayList<Impiegato> Ricerca(int risultati,
      ArrayList<Class<?>> mansione)
  {
    ...
    if (impiegati.size() > 0)
    {
      ...
      System.out.format("Risultato sulla ricerca di impiegati di tipo : %s%n",
          mansioni);
    }
  ...
  public String toString()
  {
    ...
    return String.join(",", this.nome, this.cognome, this.codiceFiscale,
        this.sedeLavorativa, this.inizioAttivita.toString(), dataFineAttivita)
        .toLowerCase();
  }
